...
import tls from "tls";
import { DomainSnapshot, getCanonicalBase } from "./storage";

// ============================================================================
// Snapshot creation
//
// Builds a DomainSnapshot by querying RDAP, DNS, TLS, and infrastructure
// sources for a domain. Extracted from POST /api/snapshot so the on-demand API
// route and the monitor worker share one implementation. Pure data collection:
// no licensing, validity, or persistence concerns — callers own those.
// ============================================================================
...
/**
 * Creates a complete DomainSnapshot for a domain by querying RDAP, DNS, TLS,
 * and infrastructure sources in parallel and merging the results onto the
 * canonical base. Deterministic data collection only — the caller is
 * responsible for licensing checks, validity checks, and persistence.
 */
export async function createSnapshot(domain: string): Promise<DomainSnapshot> {
